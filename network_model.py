...
def run_network(aj, agents, norm, history=min_track_length):
    if (len(aj.history)>history):
        ...
        x_dir = np.argmax(sess.run(tf.nn.softmax(result[0][0][-1][0])))
        y_dir = np.argmax(sess.run(tf.nn.softmax(result[0][0][-1][1])))

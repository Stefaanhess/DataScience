...
def run_network(aj, agents, norm):
    if (len(aj.history)>50):
        track_continuous = np.expand_dims(np.array(aj.history[-50:]), axis=0)
        track_discrete = np.expand_dims(digitize_track(np.array(aj.history[-50:])), axis=0)
        ...
        if (predict_all == True and len(aj.history)>50):
